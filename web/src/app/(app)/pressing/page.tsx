...
export default function PressingPage() {
  ...
  const { data: untreatedClients } = useQuery({
    queryKey: ['olive-untreated-clients'],
    queryFn: async () =>
      (
        await api.get<{ items: UntreatedClientRow[] }>('/olive-entries/client-board', {
          params: { untreatedOnly: true, limit: TABLE_FETCH_LIMIT },
        })
      ).data.items,
  });

  const { data: byClient } = useQuery({
    queryKey: ['pressing-by-client'],
    queryFn: async () => (await api.get<PressingClientRow[]>('/pressing/by-client')).data,
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['pressing-by-client'] });
      qc.invalidateQueries({ queryKey: ['olive-untreated-clients'] });
  ...
  function openWeighings(row: PressingClientRow) {
    setWeighingsClient({
      ...
      firstName: row.clientName.split(' ')[0] ?? '',
        ...
        <form onSubmit={onSubmit} className="grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            ...
            {!readOnly && (untreatedClients?.length ?? 0) === 0 ? (
              <p className="mt-1 text-xs text-[var(--app-text-dim)]">لا يوجد زبائن غير معالجين حالياً</p>
            ) : null}
          </div>
          <Input name="oilQuantityL" label="كمية الزيت (لتر)" type="number" step="0.01" required />
          <div>
            ...
            {formClientId && pricePerQuintal > 0 && (
              <p className="mt-1 text-xs text-stone-500">
                {formatNumber(pricePerQuintal)} دج/ق × {formatNumber(selectedWeight / 100, 2)} ق (
                {formatNumber(selectedWeight)} كغ)
              </p>
            )}
          </div>
          <Input name="region" label="المنطقة" />
          <Input name="zayat" label="الزيات" />
          <Input name="aidAmount" label="المساعدة" type="number" step="0.01" defaultValue={0} />
          ...
          <div className="md:col-span-2">
            <Button type="submit" loading={createMutation.isPending} disabled={!selectedEntryId}>
              حفظ
            </Button>
          </div>
          </fieldset>
              ...
              {rows.map((r, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 font-mono font-bold tabular-nums">{r.clientNumber}</td>
