...
export function ClientCard({ item, oliveType, highlighted, dimmed, onPress, onPrint }: Props) {
  ...
  return (
    ...
    >
      ...
      <View style={[styles.footer, { borderTopColor: c.border }]}>
        <Text style={[styles.footerText, { color: accent }]}>تفاصيل الأوزان</Text>
        <View style={styles.footerActions}>
          {onPrint ? (
            <Pressable
              onPress={onPrint}
              hitSlop={10}
              style={[styles.printChip, { backgroundColor: `${accent}18`, borderColor: `${accent}44` }]}
            >
              <Icon name="print" size={16} color={accent} />
            </Pressable>
          ) : null}
          <Icon name="chevron-back" size={20} color={accent} />
        </View>
      </View>
    </Pressable>
  );
...
const styles = StyleSheet.create({
  ...
  footerActions: { flexDirection: 'row-reverse', alignItems: 'center', gap: 10 },
  printChip: {
    width: 34,
    height: 34,
    borderRadius: 17,
