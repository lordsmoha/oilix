...
export default function BoardScreen() {
  ...
  const { data, isLoading, refetch, isRefetching } = useQuery({
    ...
    queryFn: async (): Promise<ClientBoardRow[]> =>
      (
  ...
  function openEntry(client?: ClientBoardRow) {
    router.push({
      pathname: '/(app)/entry',
      params: {
        oliveType,
  ...
  return (
    <Screen padded={false} edges={['top']}>
      ...
      <AppHeader
        ...
        subtitle={`${meta.emoji} ${rows.length} زبون · ${new Intl.NumberFormat('ar-DZ').format(Math.round(totalWeight))} كغ`}
        userName={user?.firstName || user?.username}
        accent={meta.color}
        onHistory={() => router.push('/(app)/history')}
        onLogout={() => void clearSession().then(() => router.replace('/'))}
      />
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <EmptyState
                icon="people-outline"
                title="لا يوجد زبائن"
                message="لا زبائن لهذا النوع — اضغط + لإضافة تصفية مباشرة"
              />
            }
            renderItem={({ item }) => (
              <ClientCard
      ...
      >
        ...
        >
          ...
          <Text style={styles.fabLabel}>تصفية جديدة</Text>
        </LinearGradient>
      </Pressable>
    </Screen>
  );
